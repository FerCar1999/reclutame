    ...
    class HabilidadEmpleo extends Validator
    {
        ...
        {
            $sql = "SELECT * FROM habilidad_empleo INNER JOIN habilidad ON habilidad.codi_habi = habilidad_empleo.codi_habi WHERE codi_empl=? AND esta_habi_empl=1";
            $params = array($this->codi_empl);
            return Database::getRows($sql, $params);
        }
        public function verificarAptitudUsuario($usua, $habi)
        {
            $sql = "SELECT codi_habi FROM habilidad_usuario WHERE codi_usua=? AND codi_habi=? AND esta_habi_usua=1";
            $params = array($usua, $habi);
